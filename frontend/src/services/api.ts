...
export const authAPI = {
  ...
  updateRole: (role: string) => api.put("/auth/updaterole", { role }),
  ...
  resetPasswordWithOTP: (email: string, otp: string, newPassword: string) =>
    api.put("/auth/resetpasswordwithotp", {
      email,
      otp,
...
export const productAPI = {
  ...
  getSellerStats: (sellerId: string) =>
    api.get(`/products/seller/${sellerId}/stats`),
};
...
export const cartAPI = {
  ...
  removeFromCart: (productId: string) => api.put(`/cart/item/${productId}`),
...
export const accountAPI = {
  ...
  withdrawFunds: (amount: number) =>
    api.post("/account/withdraw-funds", { amount }),
